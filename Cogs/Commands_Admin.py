...
class Commands_Admin(lib.commands.GroupCog, name="admin"):
  ...
  @lib.app_commands.describe(fruits='fruits to choose from')
  ...
  async def spawnmonster(self, interaction: lib.discord.Interaction, rarity: lib.Choice[str], gamemode: lib.Choice[str]) -> None:
    ...
    rGamemode = await self.bot.db_pool.fetchrow(lib.qGameModes.SELECT_GAMEMODE, gamemode)
    embed, view, channel = await lib.Battle_Functions.create_battle(self, 10, rGamemode, rarity)
