...
class Light_Button(lib.discord.ui.Button):
    ...
    async def callback(self, interaction: lib.discord.Interaction):
        ...
        content, damage, monster_killed, to_update = await self.view.Battle.handler_Attack(cSlayer, "l")
        #On répond au joueur
...
class Heavy_Button(lib.discord.ui.Button):
    ...
    async def callback(self, interaction: lib.discord.Interaction):
        await interaction.response.defer(ephemeral=True)
        cSlayer = await self.view.bot.ActiveList.get_Slayer(interaction.user.id, interaction.user.name)
        content, damage, monster_killed, to_update = await self.view.Battle.handler_Attack(cSlayer, "h")
        #On répond au joueur
        await interaction.followup.send(content=content, ephemeral=True)
        if self.view.Battle.stats["kills"] >= int(self.view.bot.Variables["battle_kills_before_escape"]):
            self.view.Battle.end = True
...
class Special_Button(lib.discord.ui.Button):
    ...
    async def callback(self, interaction: lib.discord.Interaction):
        await interaction.response.defer(ephemeral=True)
        cSlayer = await self.view.bot.ActiveList.get_Slayer(interaction.user.id, interaction.user.name)
        content, damage, monster_killed, to_update = await self.view.Battle.handler_Attack(cSlayer, "s")
        #On répond au joueur
        await interaction.followup.send(content=content, ephemeral=True)
        if self.view.Battle.stats["kills"] >= int(self.view.bot.Variables["battle_kills_before_escape"]):
            self.view.Battle.end = True
