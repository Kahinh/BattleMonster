...
def create_embed_profil_attack(cLoadout_or_cSlayer, avatar, hit, cLoadout2=None):
    ...
    for statistic in cLoadout_or_cSlayer.stats:
        if f"_{hit}" in statistic:
            print(statistic, cLoadout_or_cSlayer.cSpe.adapt_min(bot.Statistics[statistic[:-2]].cap_min, bot.Statistics[statistic[:-2]].name, cLoadout_or_cSlayer.stats), cLoadout_or_cSlayer.stats[statistic])
