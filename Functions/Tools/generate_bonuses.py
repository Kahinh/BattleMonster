    ...
    for _, cStatistic in bot.Statistics.items():
        for sub_division in cStatistic.sub_division():
            if cStatistic.cap_min is not None:
                dict_data[sub_division] = float(max(cSpe.adapt_min(cStatistic.cap_min, cStatistic.name, dict_data), dict_data[sub_division]))
            if cStatistic.cap_max is not None:
                dict_data[sub_division] = float(min(cSpe.adapt_max(cStatistic.cap_max, cStatistic.name, dict_data), dict_data[sub_division]))
    return dict_data